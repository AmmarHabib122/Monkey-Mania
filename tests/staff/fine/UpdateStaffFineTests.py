...
class TestStaffFineUpdate(SetUpDataClass):
    def test_user_with_no_branch_update_staff_fine(self):
        ...
        response = self.client.patch(url, self.test_staff_fine_1, format = 'json') #admin update Branch
    ...
    def test_user_with_a_branch_update_staff_fine(self):
        ...
        self.test_staff_fine_1.pop('staff')
        response = self.client.patch(url, self.test_staff_fine_1, format = 'json') #manager update StaffFine
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_user_with_no_permission_update_staff_fine(self):
        ...
        self.test_staff_fine_1.pop('staff')
        response = self.client.patch(url, self.test_staff_fine_1, format = 'json') #waiter update Branch

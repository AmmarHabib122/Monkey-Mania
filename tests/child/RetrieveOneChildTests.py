...
class TestChildRetrieve(SetUpDataClass):
    def test_user_with_no_branch_get_child(self):
        ...
        url = reverse('Get_Child', kwargs={'pk': self.child_1.id})  #admin get Child
    ...
    def test_user_with_a_branch_get_child(self):
        ...
        url = reverse('Get_Child', kwargs={'pk': self.child_1.id})  #manager get  Child
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        self.authenticate(self.reception_user_1)
        url = reverse('Get_Child', kwargs={'pk': self.child_1.id})  #recepetion get Child
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_user_with_no_permission_get_child(self):
        ...
        url = reverse('Get_Child', kwargs={'pk': self.child_1.id})  #waiter get child
        response = self.client.get(url)
